...
vec3_t barycentric_weights(vec2_t a, vec2_t b, vec2_t c, vec2_t p) {
    ...
    float area_parallelogram_abc = (ac.x * ab.y - ac.y * ab.x); // || AC * AB ||

    // Alpha is the area of small parallelogram/triangle PBC divided by the area of full parallelogram/triangle ABC
    float alpha = (pc.x * pb.y - pc.y * pb.x) / area_parallelogram_abc;

    // Beta is the area of small parallelogram/triangle APC divided by the area of full parallelogram/triangle ABC
    float beta = (ac.x * ap.y - ac.y * ap.x) / area_parallelogram_abc;

    // Weight gamma is the rest
...
void draw_filled_triangle(
    ...
    // Create three vector points after sort
...
void draw_texel(int x, int y, uint32_t* texture,
    ...
    if (interpolated_reciprocal_w < z_buffer[(window_width * y) + x]) {
        ...
        // Update the z-buffer value with the 1/w of current pixel
        z_buffer[(window_width * y) + x] = interpolated_reciprocal_w;
    }
